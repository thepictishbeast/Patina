...
| ✅ `15-ownership-and-moves` → `15-ownership` + `15b-moves` + `15c-copy-and-clone` (done `769f282` — the LAST work row; stack/heap → one sentence + Book §4.1; NEW E0425-after-drop + working-move-first examples; 15c re-reads 15b's E0382; coupling → 15b) | 1517 | split | 6 | 0 | → **Ownership, Scope, and Drop**<br>→ **Moves — assignment hands ownership over**<br>→ **Copy and Clone** |
