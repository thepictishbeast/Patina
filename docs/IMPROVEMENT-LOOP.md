...
- **⚠ The single-tick UN-GATED backlog is running thin (2026-07-08) — where next?** After the v0.4 push the app is feature-complete + polished on every solo-shippable axis: the IDE (unified fullscreen editor + Sandbox + full explorer toolkit: collapsible phases / reveal-on-open / filter / fold-all), Practice (predict-first, hints, gamify, recall), the roadmap (now with animated per-stage progress), Learn hub, lessons (grouped + filter), book (search + cleanup), glossary (search), quizzes, cheatsheets. Content is **saturated** (41 error codes + all common runtime panics). Tests are green (92). The remaining high-value work is **gated or multi-surface**, and I'd like your call on which to prioritise:
  ...
  3. **Spaced-repetition upgrade** — the RECALL chips exist; a real SM-2-style review schedule would lift retention. Medium size.
  Until you steer, I'll keep doing tasteful #40 polish + any on-device feedback from dev.42 — but flagging so I don't manufacture low-value churn.
...
- ✅ **Fixed hidden CI-red: two dead concept chips + a stale TUI test (`c1aeaa2`, 2026-07-08).** Ran the FULL `check.sh` (not just per-tick e2e/gui-transform) and it surfaced 3 red things the loop had been silently carrying: (1+2) two DEAD concept chips — the exercises added earlier this session, `basics/07b_duplicate_fn_name` ("duplicate-definition") and `ownership/02b_deref_non_reference` ("dereference-non-reference"), had no glossary term, so their Practice concept chip showed nothing (caught by the `golden_corpus` test). Added two proper conceptual glossary terms ("duplicate definition", "dereferencing"), no exercise-fix handed, no language analogies; a re-scan confirms EVERY exercise concept now resolves + live `/api/glossary` returns both. (3) the `rpro-tui` book-reader test asserted the OLD listing callout ("Read this code listing") — I'd reworded rpro-book's callout offline-first an earlier tick and updated rpro-book's tests but missed this shared-cleaner TUI test; fixed the assertion. CONTENT_VERSION 42→43. **Full `check.sh`: 14 passed, 0 failed — branch mergeable.** ⚠ LESSON: run `cargo test --workspace` (or check.sh) periodically, not just e2e+gui-transform — golden_corpus (concept→glossary resolution) and TUI render tests aren't in the per-tick set, so red accrued unseen. Mobile picks up the terms at next APK build.
- ✅ **Killed the long-standing web.spec store-pollution flake (`e6a2cf7`, 2026-07-08).** The intermittent `web.spec:32` failure (up to ~3/4 full runs this tick — a HARD failure, not just flaky) was root-caused, not deferred again. NOT a bad assertion: the four `compile` specs mutate ONE shared store under parallel workers, and a passing run marks whatever's CURRENTLY selected Done — so recall-chip's passing FIX run, if it fires while web/dev-diag just selected their exercise, marks THAT exercise Done → web's force-select 409s. Instrumentation proved it (`SEL STATUS 409` even for the trait exercise no spec deliberately passes) → it's a concurrent-store-write race, unfixable by exercise choice. **Fix** (scripts/e2e.sh): on a full run, run the `ui` project parallel (fast) but the `compile` project SERIALLY (`--workers=1`) so no two store-writers overlap; a targeted run keeps the single fast pass. Also pointed web's tier test at `traits/03_trait_object_dyn` (never Done) + a re-pin-before-assert. Verified **4/4 consecutive full runs GREEN** (ui 88 + compile 10, web.spec ✓ every time; ~26s total, not slower — retries no longer burned on the flake); targeted runs still 10/10. **The e2e suite is now reliably green — no known flakes.**
- 📱 **RELEASED v0.4.0-dev.44 — search everywhere, offline (mobile `02cc147`, 2026-07-08).** Batched the post-dev.43 search work into a silent prerelease: the Learn-hub **"search everything"** box + two offline fixes that only reach the phone via a release — **Book search** (was silently "no matches" on mobile) and **RECALL spaced-repetition** (`/api/review` had no mobile handler → chips never showed). All GUI-shell + two seam fixes → store unchanged (**76 exercises**, CV 42). Built locally; bundle-verified the unzipped APK: versionName **0.4.0-dev.44** (versionCode 10358), hub search (`hubsearch`/`hubSearch`/`hubresults`/"Search everything"), lesson search (`lessonbodyhits`), seam fixes (`bookSearchJson`+`reviewJson` in dex, `Java_..._Seam_reviewJson` in the arm64 `.so`), 76 exercises. **Anon download HTTP 206, 26.81MB, uploaded.** PushNotification (suppressed — terminal focused); NO email. Release: github.com/thepictishbeast/Tempered-Studio-Mobile/releases/tag/v0.4.0-dev.44. Next dev.N = 10359.
- ✅ **Learn hub: a unified "search everything" box — the search capstone (`52bdf59`, 2026-07-08).** One box on the Learn hub finds a concept across ALL bundled material at once — lessons + the Rust Book + cheatsheets (server `?q=`, ranked with snippets) plus the glossary (client-filtered) — categorized under 🎓 Lessons / 📖 Book / 📋 Cheatsheets / 📚 Glossary, click a hit to open it. Find where a concept lives without first picking a section. Debounced (220ms) + stale-seq guard; results replace the card grid while active, cards return on clear/Esc; nonsense → no-match note. Fully offline (every endpoint mirrored on mobile — see the parity audit). Purely additive (per-view searches stay). XSS-safe (esc + `count | 0`). GUI-shell only, no CV bump. Verified: transform + JS-parse clean; live-drove 390×844 ("borrow" → **54 hits across all 4 categories** with snippets; nonsense → no-match; clear → cards restored; 0 errors) + screenshot; new `learn-hub.spec.js` guard + Learn-hub a11y ×2 — **21 passed**. **🎉 Search story complete: per-view (lessons body / book / glossary) + a global hub search, all offline everywhere.**
- ✅ **mobile: RECALL spaced-repetition works offline — `/api/review` (mobile `b75d605`, 2026-07-08).** Found via a systematic audit (GUI's 14 `/api/` calls vs the 10 the mobile server handled): `run`/`sandbox`/`select` are POST → the `AndroidSeam` JS bridge (expected), but **`/api/review` (a GET) had no mobile handler** → the gui's `fetch('api/review')` fell through to null → the RECALL chips (spaced repetition of overcome error codes) NEVER showed on the phone, though on-device `recordRun` already tracks reviews. Fixed by adding `Seam.reviewJson` — a direct mirror of rpro-serve's `review_handler` reading the SAME `progress.reviews` (shared **rpro-state** crate), returning `{due,mastered,tracked}`; declared the native method + routed `/api/review` in MainActivity. No gui change (already calls it). Reuses tested SRS logic — no reimplementation. No version bump (batches to next dev.N). Verified: `cargo check` clean; cargo-ndk cross-build + gradle **BUILD SUCCESSFUL**; the arm64 seam `.so` exports `Java_..._Seam_reviewJson` + the dex carries the call. **Offline-parity audit: the GUI's Learn/practice GETs are now ALL mirrored on mobile** (book/lessons/quizzes/cheatsheets search + review); only POST endpoints (run/sandbox/select) go via the Termux/JS bridge by design.
- ✅ **mobile: Book full-text search fixed offline on the phone (mobile `b2a6363`, 2026-07-08).** The gui's Book search box (`runBookSearch` → `/api/book?q=`) worked on desktop but SILENTLY returned "no matches" on the phone — the mobile handler ignored `?q=` and returned the TOC (`{chapters}`, no `hits` key). Same offline gap the lesson search just closed, now for the Book. Fixed as a PURE-JAVA mirror of desktop `rpro_book::Book::search` (the 33 chapters are plain seeded `.md`, no seam/NDK change): `/api/book?q=TERM` → `{"hits":[{chapter,title,count,snippet}]}` ranked by count — the exact shape the gui renders (`data-ch`=chapter). Reuses `countOccurrences`+`mdSnippet`; adds `bookTitle` which reads a heading of ANY level (Book chapters lead with `## `, so the lesson `# `-only `mdTitle` would fall back to the id — `bookTitle` matches rpro-book's `Chapter::title` so hit titles equal the TOC). No gui change (the box already calls `/api/book?q=`). No version bump (batches to the next dev.N after dev.43). Verified: `bookTitle` JVM-tested (## / # / ### / no-heading / indented all pass); gradle **BUILD SUCCESSFUL**; dex carries `bookSearchJson`+`bookTitle`; desktop shape re-confirmed identical (`{chapter,count,snippet,title}`, `##`-heading titles). **Search now works offline everywhere for BOTH lessons and the Book.**
- 📱 **RELEASED v0.4.0-dev.43 — lesson full-text search + explorer/journey polish (mobile `b43240e`, 2026-07-08).** Batched the post-dev.42 work into a silent prerelease (Obtainium; STABLE v0.4 gated): the complete **lesson full-text search** (offline — Lessons box searches lesson TEXT not just titles; "📄 Also found in the text of" w/ counts+snippets; both servers `?q=`), the IDE **fold-all/unfold-all** control, the **per-stage Journey progress bars**, and the gold **"Phase complete!" milestone**. All GUI-shell + the one Java seam change → store unchanged (**76 exercises**, CV 42). Built locally (`build-apk.sh`, 7s); bundle-verified the unzipped APK: versionName **0.4.0-dev.43** (versionCode 10357), lesson-search GUI (`lessonbodyhits`/`lbh-list`/"Also found in the text") + seam search (`countOccurrences`/`mdSnippet` in dex), `idefoldall`/`roadprog`/`celebratePhase`/`maybeCelebratePhase` tokens, 76 exercises, arm64 seam `.so` exports workspaceJson. **Anon download HTTP 206, 26.76MB, uploaded.** PushNotification (suppressed — terminal focused); NO email (dev.N silent). Release: github.com/thepictishbeast/Tempered-Studio-Mobile/releases/tag/v0.4.0-dev.43. Next dev.N = 10358.
- ✅ **Lessons GUI: full-text body search — LESSON SEARCH DONE 3/3 (`2eb6fb2`, 2026-07-08).** The final piece (after desktop `203ac7c` + mobile `2f241fd`). The Lessons-list filter box now searches lesson TEXT, not just titles: keeps the instant client-side title filter AND, debounced (200ms + stale-response seq guard), queries `/api/lessons?q=` to surface body matches under a "📄 Also found in the text of" section (match count + snippet, click to open) — so "borrow" finds both lessons named for it AND those that discuss it. Body hits exclude already-shown title matches; no-match note hides when EITHER matches. **Graceful on older APKs** (server ignores `?q=` → no `hits` key → body section stays hidden, title filter still works). Own `.lbh-list` class (not `.booktoc`) keeps it decoupled from the title-filter DOM/tests. GUI-shell only, no CV bump. Verified: transform + JS-parse clean; live-drove 390×844 ("borrow" → 3 title + 24 body-only hits w/ snippets; gibberish → no-match + body hidden; clear → 12 stages restored; 0 errors) + screenshot; new `lesson-nav.spec.js` guard + decoupled filter test + Lessons a11y ×2 — **31 passed**. **🎉 Lesson full-text search complete, offline everywhere (desktop + mobile + GUI).** Ships in the next dev.N.
- ✅ **mobile seam: mirror the `?q=` lesson/quiz/cheatsheet search on-device (mobile `2f241fd`, 2026-07-08).** Piece 2/3 of lesson search. The offline HARD rule: any new `/api/` param the GUI will call MUST be mirrored in the mobile embedded server first, or search would silently return the unfiltered list on the phone. So the Java `mdCollection` (MainActivity) now handles `?q=TERM` with the SAME shape as desktop `md_collection`: `{"hits":[{id,title,count,snippet}]}` ranked by count; checked before `id` (never joins a path); blank → list; no-match → empty hits. New pure static helpers `countOccurrences` + `mdSnippet` mirror the Rust logic (index-clamped preview so it never throws). No version bump (batching; GUI piece next). Verified: the two helpers **JVM-tested in isolation** (count/snippet/ellipsis pass); `gradle assembleDebug` **BUILD SUCCESSFUL**; the compiled dex carries `countOccurrences`/`mdSnippet` + the `hits` shape. **Both backends (desktop + mobile) now support lesson search → the GUI piece (3/3) can land next tick and work offline everywhere.**
- ✅ **serve: full-text search for lessons/quizzes/cheatsheets — `?q=` (`203ac7c`, 2026-07-08).** The Rust Book was searchable (`/api/book?q=`) but the Patina lessons (the PRIMARY teaching material) weren't. Added a `?q=TERM` branch to the shared `md_collection` handler → `/api/lessons`, `/api/quizzes`, `/api/cheatsheets` all gain full-text search at once, mirroring the book: `hits {id,title,count,snippet}` ranked by match count. New `md_snippet` builds a plain-text preview around the first match (collapses whitespace, drops loud md noise, char-boundary-safe so non-ASCII can't panic). Search checked BEFORE the `id` branch → never path-joins; blank term falls through to the list; no-match → empty `hits` (never the list). **Server-side FOUNDATION only** — GUI + mobile-seam wiring (offline-everywhere) follow in later ticks; endpoint inert until then. On Paul's "Rust improvements" v0.4 focus; fills the real gap flagged to Paul (lessons unsearchable). No content change → no CV bump. Verified: `cargo test -p rpro-serve` **27/27** (new `lessons_full_text_search_returns_ranked_hits`); clippy `-D warnings` clean (fixed needless_pass_by_value + redundant_closure); fmt clean; live end-to-end (`?q=immutable`→7 ranked hits w/ clean snippets, `?q=predict` on quizzes→11, plain list still 78). **Also this tick: the throttled doc commit `acd0ed4` finally pushed (throttle cleared).** Re-confirmed first that lessons/quizzes/roadmap are otherwise complete (quiz list already shows ✓/count; quiz-taking is predict-first + polished).
- ✅ **Practice #40: a gold "Phase complete!" milestone celebration (`4a4ed6f`, 2026-07-08).** Finishing the LAST exercise of a whole phase used to land with the same green per-exercise popup. Now, when a pass makes its phase fully done, a distinct GOLD milestone ("🏆 Phase complete! You finished <Phase> — every exercise done.") fires instead — a real reward for clearing a topic; once per phase (deduped in `ts-phase-celebrated`), only on genuine completion. Detection is client-side from the exercises list already fetched on advance (phase = id prefix, all in-phase `done`) — no extra request, no server/mobile-seam change; wired into BOTH advance paths (web `/api/run` + mobile Termux record-run). `celebratePhase` reuses `#celebrate` with a `.milestone` class (gold, larger, 3.6s, honours `prefers-reduced-motion`). This tick FIRST re-confirmed the app is complete (RECALL SRS is server-side → multi-surface, deferred; lessons already have prev/next nav + filter) before finding this genuinely-missing motivational moment. GUI-shell only, no CV bump. Verified: transform + JS-parse clean; live-drove 390×844 (logic fires-once→dedupes→partial-never, persisted; gold popup "You finished Control Flow") + screenshot; new `motion.spec.js` guard + the advance-path specs (recall-chip, gamify completion, motion) green — **full suite 95 passed** (lone web.spec failure = pre-existing parallel store-pollution flake, 3/3 in isolation).
- ✅ **Journey #40: an animated per-stage progress bar on the roadmap (`41918b0`, 2026-07-08).** The roadmap listed lessons per stage but only showed a "N/M read" badge once you'd read ≥1 — a new learner saw no progress cue. Every stage now carries an always-present slim progress bar + count that animates 0→read-fraction on view-open (snaps under `prefers-reduced-motion`), turning green + ✓ when complete. The journey is measurable from day one and visibly fills as you read. Pure client-side (reuses `readLessonSet` + the lesson list; no API/mobile-seam change → offline everywhere). GUI-shell only, no CV bump. Verified: transform + JS-parse clean; live-drove 390×844 (stage 1 1/1 ✓ 100% green, Foundations 3/9 33%, rest 0/N; 0 page errors) + screenshot; new `lesson-nav.spec.js` guard + roadmap a11y ×2 themes — **30 passed**. This tick FIRST audited the whole app (Practice/IDE/roadmap/Learn hub/lessons+filter/book+search/glossary+search) and confirmed it's **feature-complete + polished on the un-gated axis** — see the new Open-Questions note flagging the thinning single-tick backlog.
- ✅ **IDE #43: a fold-all / unfold-all control for the explorer (`4e4bc5a`, 2026-07-08).** Rounds out the explorer toolkit (collapsible phases + reveal-on-open + filter) with a one-click fold/unfold of EVERY phase, beside the filter box: any phase open → collapse all (the whole 12-phase curriculum becomes a tidy one-screen overview with count badges) → click again to expand all. Persists (`ts-ide-groups`); the button re-labels to its next action (▾ collapse / ▸ expand), synced each render. `ideMakeGroup` stamps `data-gkey` so `ideFoldAll` persists every group in one pass; chevron glyphs reuse the header ones (present in the offline Noto subset — no tofu). This tick FIRST verified content is saturated (41 error codes + all common runtime panics covered — E0412 folds into the already-covered E0425 in modern rustc) and that the remaining web.spec/recall-chip flake is a deep parallel-shared-store race with no clean reset API (correctly deferred, non-blocking). GUI-shell only, no CV bump. Verified: gui-transform + JS-parse clean; live-drove 390×844 (collapse→0 open/0 rows, expand→12 open, persists across reload, 0 page errors) + screenshot; new `ide.spec.js` guard + a11y ×2 themes — **30 passed**. **#43 explorer toolkit is now complete.**
- ✅ **Fixed the standing red: dev-diag-jump strict-mode failure (`aef97cc`, 2026-07-08).** The dev-diag-jump suite had been red for 5+ ticks — I'd repeatedly flagged it "pre-existing, own tick." Root-caused it this tick: NOT a feature regression. The exercise (`traits/03_trait_object_dyn`) compiles to a primary **E0308** AND a cascading **E0282** (type-annotations-needed), so the Assist panel renders multiple `.diagcode` spans; `runInAssist` asserted on a bare `locator('.diag .diagcode')` which matches >1 element and trips Playwright **strict mode** on `toContainText` — the ONLY diag locator in the file missing `.first()` (every other already had it). Confirmed the feature is healthy first (live: select 200, run → `.diag` shows E0308 + jumpline L32, 0 page errors), then fixed by asserting on `.first()` (the primary E0308). dev-diag-jump **5/5**; **full e2e 92 passed, 0 hard failures** (the lone web.spec "flaky" passes on retry — known cross-spec store pollution). Test-only, no gui change, no CV bump. The suite is now genuinely green.
- 📱 **RELEASED v0.4.0-dev.42 — the unified fullscreen IDE + explorer overhaul (mobile `952019c`, 2026-07-08).** Batched ~6 ticks of IDE work since dev.41 into a silent prerelease (Obtainium auto-updates; STABLE v0.4 stays gated). Bundles: fullscreen IDE actually fills the viewport (`2949f85`); the 🧪 Sandbox ↔ 🛠 IDE unification — scratch files live in the IDE explorer + the standalone Sandbox view retired + dead code excised (`b1b1598`/`ef3e6ce`/`a386552`); the file-explorer overhaul — collapsible phase groups (`928a51e`), reveal-folded-open-file (`b8ef8f5`), and the cross-phase filter box (`e7f1d34`). All GUI-shell → store unchanged (same **76 exercises**, CONTENT_VERSION 42). Built locally (`HOME=/home/paul bash build-apk.sh`, 7s, self-hosted runner down); bundle-verified the unzipped APK: versionName **0.4.0-dev.42** (versionCode 10356), the new IDE tokens present (idefilter/ideFilterTree/ide-treelist/ideMakeGroup/ideAppendScratchGroup/ide-full-on), 76 exercises, arm64 seam `.so` exports workspaceJson + workspaceFileJson. **Anon download HTTP 206 (range), 26.75MB, state uploaded.** PushNotification (suppressed — terminal focused); NO email (dev.N silent). Release: github.com/thepictishbeast/Tempered-Studio-Mobile/releases/tag/v0.4.0-dev.42. Next dev.N = 10357.
- ✅ **IDE #43: a filter box to jump to any file by name across phases (`e7f1d34`, 2026-07-08).** With 76 files across ~16 collapsible phases, finding one meant scrolling + un-folding. The explorer now has a filter `<input>` at the top: typing shows only matching rows (their phase group revealed regardless of fold state; non-matching rows + empty phases hidden), so you jump straight to any file; clearing/Esc restores the folded view untouched. `#idetree` now holds a persistent `#idefilter` above an inner `#idetreelist` that `ideRenderTree` fills (box survives re-renders; an active filter re-applies after a re-render). `ideFilterTree` is purely visual — never re-fetches, never persists, so `ts-ide-groups` fold prefs stay intact; the "＋ New scratch" action is excluded from matches; matching groups force their body open over the collapsed rule. GUI-shell only, no CV bump. Verified: gui-transform + JS-parse clean; new `ide.spec.js` guard (match one file across phases; nonsense query hides all; clear restores) — **9/9**; live-drove 390×844 ("fn" surfaced 6 files across 4 phases, others hidden, 0 page errors) + screenshot. **#43 is now solid** (collapsible groups + reveal-on-open + filter). Next: consider cutting **dev.42** (big unreleased IDE batch) so Paul can test on-device.
- ✅ **IDE #43: opening a file reveals it when its phase group is folded (`b8ef8f5`, 2026-07-08).** Follow-up to the collapsible groups: a restored or jumped-ahead file could sit inside a COLLAPSED phase, so its open-highlight was hidden behind the fold. `ideMarkOpenInTree` now expands the open row's group + scrolls it into view (`block:'nearest'`). DOM-only — never writes the persisted toggle, so explicit fold choices survive the next full render. GUI-shell only, no CV bump. Verified: gui-transform + JS-parse clean; new `ide.spec.js` guard (collapse the current group → open its file → group re-expands, row visible + marked open) — **8/8**; live smoke 390×844 (0 page errors; opened file visible/open/marked) + screenshot (open file highlighted in an expanded 01-BASICS).
- ✅ **IDE #43: collapsible phase groups in the file explorer (`928a51e`, 2026-07-08).** The explorer listed all 76 exercise files across ~16 phases as one long scroll — brutal on a phone. Each phase (and the 🧪 Scratch group) is now a foldable group: a header BUTTON with a chevron + a file-count badge. The phase holding the CURRENT exercise stays open, the rest collapse (so a long tree opens on "where you are" — mirrors the Lessons list's stage grouping); explicit toggles persist per group in `ts-ide-groups`; Scratch defaults open. New shared helper `ideMakeGroup(key,label,count,collapsed)→{g,body}` (used by both `ideAppendScratchGroup` and `ideRenderTree`, no duplicated scaffolding); rows stay `.ide-file` under `#idetree` so `ideMarkOpenInTree` + the answer-leak guards are unaffected; chevron rotation honours `prefers-reduced-motion`. GUI-shell only, no CV bump. Verified: gui-transform + JS-parse clean; live-drove 390×844 (12 headers, 10 collapsed / 2 open; 76 rows but only **9 visible** = the open phase; expanding a group 9→18; a toggle survives reload) + screenshot; new `ide.spec.js` guard (**7/7**) + full e2e **86 passed** (the 2 dev-diag-jump failures + recall-chip flake are PRE-EXISTING). **First bite of #43;** further ideas: active-file scroll-into-view when its group is collapsed, a filter/search box.
- ✅ **Dead standalone-Sandbox subsystem excised — #42 fully DONE (`a386552`, 2026-07-08).** Cleanup follow-up to `ef3e6ce`: deleted the code the reroute left unreachable — ~10.7KB JS (`renderSandbox` + `sbxRenderFilesBar`/`sbxSwitchTo`/`sbxNewFile`/`sbxDeleteActive`/`sbxSetDoc`; the old run path `runSandbox`/`sbxRender`/`sbxView` + the `sbxPending` branch & decl in `__termuxResult`) and two dead CSS blocks (`.sbxbar`/`.sbxhost`/`.sbxout`/`.sbxfiles`/`.sbxfile*`). Net `gui/index.html` −195/+6 lines. Removal done via a content-anchored script that asserts each anchor is unique (no line-number fragility) + a post-check that all 10 symbols are gone. The IDE scratch Run is untouched — it goes through `runCode → _runCbs` (a SEPARATE `__termuxResult` branch) on web (`/api/sandbox`) and mobile (Termux op `sandbox`); kept the live store bits the IDE uses (`sbxLoadFiles`/`sbxSaveFiles`/`sbxActive`/`sbxOpenWithCode`/`SBX_DEFAULT`/`SBX_FILES_KEY`/`sbxFiles` + `SBX_KEY` old-key migration). "Shrink, don't grow" honored. GUI-shell only, no CV bump. Verified: gui-transform + JS-parse clean; live smoke 390×844 (🧪 Sandbox menu → IDE scratch, Run printed "unified-42"); full e2e **86 passed** (sandbox/runnable-code/ide/ide-scratch/a11y green) — the 4 compile-project failures (dev-diag-jump ×3, web tier ×1) are PRE-EXISTING, unrelated. **Board #42 marked completed/removed.** Next headline = **#43 file-explorer UX** (e.g. collapsible exercise-phase groups — the tree is 76 files across ~16 phases, a long phone scroll).
- ✅ **The Sandbox IS the IDE now — standalone view retired (`ef3e6ce`, 2026-07-08) — completes Paul's "same thing" directive.** Last tick folded scratch files INTO the IDE explorer; this tick removes the *second door*. Every Sandbox entry point now opens the unified IDE focused on a free-play scratch file: the 🧪 Sandbox menu entry (`showView('sandbox')`), the "✎ Open in the IDE" button under runnable lesson/book code (relabelled from "Open in Sandbox" — `sbxOpenWithCode` pushes the example as a new scratch, same route opens it), and any legacy `showView('sandbox')` call. `renderIde(entry)` gains a `'scratch'` mode (sandbox route → active scratch; normal IDE route → last-open file). The old `renderSandbox` view + its file-tab helpers (`sbxRenderFilesBar`/`sbxSwitchTo`/`sbxNewFile`/`sbxDeleteActive`/`sbxSetDoc`) are now dead — flagged in-place for excision next cleanup tick (kept one tick only because they share the mobile Termux run callback `sbxRender`/`sbxPending`, which can't be device-verified here). Progress-neutral throughout; GUI-shell only, no CV bump. Verified: gui-transform + JS-parse clean; live-drove 390×844 (🧪 Sandbox menu → IDE on a "scratch" file, Scratch group present, Save hidden/Delete shown, no `#sbxhost`, Run printed "unified-42" via the offline compiler) + screenshot; rewrote `sandbox.spec.js` (menu + legacy route land in the IDE scratch view, run there, progress untouched, no `#sbxhost`), updated `runnable-code.spec.js` (example opens in the IDE) + `a11y.spec.js` (Sandbox route = IDE editor) + `ide.spec.js` (`.rs`-only answer-leak check scoped to `data-id` exercise rows so scratch names don't trip it). Targeted suites **35 passed** (sandbox/runnable-code/ide/ide-scratch/a11y); full suite **85 passed** — the 4 `compile`-project failures (dev-diag-jump ×3, web tier ×1) are **PRE-EXISTING**, identical on clean HEAD `9fda901`, untouched by this change. **⚠ Pre-existing to investigate:** `dev-diag-jump.spec.js` fails consistently even in isolation on clean HEAD (the Assist E0### diagnostic-jump path) — not a load flake; own tick. **#42 now essentially DONE** (only the dead-code excision remains); next headline = #43 file-explorer UX.
- ✅ **IDE + Sandbox merged — scratch files live in the IDE explorer (`b1b1598`, 2026-07-08) — Paul: "the sandbox and fullscreen ide should be the same thing ... improve the way the files work with it".** Part 2 of the IDE directive. The file explorer now carries a "🧪 Scratch" group of free-play files backed by the SAME localStorage store the standalone Sandbox uses (`ts-sandbox-files`), so one environment holds BOTH the course's exercise files AND the learner's own scratch files. `ideAppendScratchGroup` builds the group first and always client-side (survives an unreachable `/api/workspace` — the tree degrades to a note instead of wiping), with a "＋ New scratch" affordance; `ideOpenScratch`/`ideNewScratch`/`ideDeleteScratch` open/create/delete in-tree (store keeps ≥1). Scratch buffers auto-persist on every edit (no disk Save), so the toolbar adapts: scratch hides 💾 Save/↺ Reset and shows 🗑 Delete, an exercise file keeps Save/Reset and hides Delete; last-open scratch restores across reloads via `st.openScratch`. Exercise files stay a progress-neutral workshop (no answer leak, no advance). Verified: gui-transform + JS-parse clean; new `ide-scratch.spec.js` (5 tests, phone 390×844: group renders, toolbar adapts, edits persist to the shared store + survive a reload, create/delete keeps ≥1, exercise files still open cleanly) **all pass** via the isolated-store harness; live-drove the whole flow at 390×844 with a screenshot (scratch open, edit persisted "saved in your browser ✓", new→mytest, delete→1, exercise file opens with Save visible/Delete hidden). GUI-shell only, no CV bump. **Still open on #42:** retire the standalone Sandbox view — route its menu entry + `sbxOpenWithCode` into this unified IDE (next bounded chunk).
- ✅ **IDE fullscreen ACTUALLY fills the screen now (`2949f85`, 2026-07-08) — Paul: "the fullscreen ide isnt fullscreen".** The IDE renders into `#docview`, a centered 680px reading column at `height:calc(100vh - 45px)`; fullscreen only set `max-width:none` + hid the header, leaving the 45px gap and brittle `calc(100vh - 118px)` editor math — never truly fullscreen. Rebuilt it as a fixed overlay: `body.ide-full-on #docview` → `position:fixed; inset:0; height:100vh`, flex column, chrome hidden; the ide-body flexes to fill, the sidebar stretches to full height, the editor is `flex:1` (fills after the header row + terminal). Verified live at 1280×800 AND 390×844: `#docview` covers the whole viewport (top:0/left:0, w==innerWidth, h==innerHeight), editor + open terminal fit, sidebar 739==739 when open; strengthened `ide.spec.js` to assert the fill; **e2e 85 passed**; seam-guard clean. GUI-only, no CV bump. **Part 1 of Paul's IDE directive** — next: merge the Sandbox into the IDE (board #42) + improve the file explorer (board #43); see memory `ide-sandbox-unify`.
- 📱 **RELEASED v0.4.0-dev.41 — visible predict verdict + polish (mobile `023233a`, 2026-07-08).** Small focused batch since dev.40: the predict-first verdict is now VISIBLE ("were you right?" was written to a collapsed element), a perfect guess earns a 🎯; the completion popup shows the friendly title not the id slug; locked badges show how-to-earn + progress; +1 exercise (76, E0428, CV42). Built locally (`gradle BUILD SUCCESSFUL`); bundle-verified the unzipped APK: versionName 0.4.0-dev.41, gui carries pv-verdict/pv-perfect/predictNailed/bgoal/nextTitle/duplicate-definition, 76 exercises incl. 07b_duplicate_fn_name, arm64 seam .so still exports the offline workspace endpoints. **Anon download HTTP 200, 26.75MB.** PushNotification (suppressed — terminal focused); no email (dev.N silent). Release: github.com/thepictishbeast/Tempered-Studio-Mobile/releases/tag/v0.4.0-dev.41
- ✅ **New exercise — two items can't share a name, E0428 (`textbook-integration`, 2026-07-08).** Confirmed the app is mature across audited surfaces (hint ladder, Sandbox, core loop) then added a distinctive uncovered beginner wall: two functions both named `greet` → E0428 "the name `greet` is defined multiple times". Teaches name-uniqueness in a scope; the fix renames one (greeting + farewell) and calls each — a warning-free solve. Original-authored, guides without handing; new concept `duplicate-definition` → 07-functions keeps the coupling a clean bijection (69/69). Verified: compile-checked wall + clean fix, `verify-exercises` 76/76, `verify-book-anchors` 120/120, CONTENT_VERSION 41→42, live-drove Practice 390×844 (predict gate + chip + read-the-lesson → 07-functions), fmt + **e2e 85 passed**, seam-guard clean. Batched for v0.4, no APK.
- ✅ **Locked badges show how to earn them + progress (`5b8b465`, 2026-07-08).** Verified the whole "stuck" path this tick — the hint ladder guides toward concept → error code → book → "apply the compiler's help line" and provably never leaks the fix (drove all 3 rungs on the E0384 exercise), and every one of the 75 exercises has complete hint metadata (book_refs + concept + solution_outline). All solid. The one real gap: the gamification panel's LOCKED badges were mystery locks (dimmed icon + name only). Gave each badge a criterion + goal (`kind`/`goal`/`need`, `test` still the award source) and rendered locked ones with progress ("12/20", "1/3 🔥") + the criterion in the tooltip; earned show "earned ✓". Moved the locked-dim to the icon so the name + progress stay readable. Verified: live-drove the panel at 390×844 (12 done → earned/locked progress all legible), extended `gamify.spec.js`; **e2e 85 passed**; seam-guard clean. GUI-only, no CV bump.
- ✅ **Completion celebration shows the friendly title, not the id slug (`f0cc9a1`, 2026-07-08).** Drove the whole win-moment (solve→run→pass→advance) end to end — it's solid (celebrates, advances, folds gamification, resets the predict gate cleanly, no stale state). One rough edge: the "🎉 Exercise complete! Next up: …" popup + the terminal's "advancing to …" line printed the raw id ("basics/02_annotation_needed") instead of the lesson title. Both run paths already fetch `/api/current` right after advancing (to load the next exercise), and that carries the next `title` — so pass it to `celebrate(nextId, nextTitle)` + use it in the terminal line, falling back to the id. Verified: live-drove a real solve of 02_annotation_needed — the popup read "Next up: Function arguments must match the parameter type" (no slug); added a `gamify.spec.js` guard; **e2e 85 passed**; seam-guard clean. GUI-only, no CV bump. Works web + mobile.
- ✅ **Predict verdict now VISIBLE + a perfect-guess celebration (`33c0eb6`, 2026-07-08).** Found a real pedagogy gap while looking for a #40 beauty touch: the predict-first payoff ("were you right?") was **invisible** — `showPrediction` wrote the verdict into `#predverdict`, which sits inside `#predictbar`, but that bar collapses to `display:none` once you lock a guess, so the verdict was written to a hidden element and never seen. Fixed by rendering the verdict into the VISIBLE locked summary (`#predictDone`, keeping its delegation-wired "change" link), and added a tasteful reward — a truly perfect prediction (outcome AND code right, or a correctly-called clean pass) earns a 🎯 + scale-pop, collapsed under prefers-reduced-motion. Verified: live-drove a real predict→run at 390×844 (green summary now shows "🎯 ✓ outcome: right · ✓ error E0384 — nailed it"), drove the logic across perfect/wrong-code/wrong-outcome/clean-pass (🎯 only on a true perfect), change link still works; added a `motion.spec.js` guard; **e2e 84 passed**; seam-guard clean. GUI-only, no CV bump.
- 📱 **RELEASED v0.4.0-dev.40 — the accumulated batch since dev.39 (mobile `9830e6f`, 2026-07-08).** ~8 loop-ticks of verified work sat unreleased while Paul tests on-device, so shipped it as a silent dev.N prerelease (Obtainium auto-updates; STABLE v0.4 stays gated). Bundles since dev.39: the offline IDE redesign + `/api/workspace` embedded-server endpoints + editor offline-draft persistence; #41 textbooks (Book directive/Listing cleanup + offline-first callout, bidirectional lesson↔Book chapter switching, reading-size, concept-pill, full glossary coverage); gamification + motion; +3 exercises (75 total, CV 36→41). Built locally (self-hosted runner down), `gradle BUILD SUCCESSFUL`. Bundle-verified the unzipped APK: versionName 0.4.0-dev.40, gui carries renderIde/LESSON_BOOK/BOOK_LESSON/cleanBookMarkdown/ideBufSave/gamifyUpdate, 75 exercises incl. the new ones, arm64 seam `.so` exports workspaceJson+workspaceFileJson, glossary.json rebuilt. **Anon download HTTP 200, exact 26.75MB match.** PushNotification (suppressed — terminal focused); no email (dev.N silent). Release: github.com/thepictishbeast/Tempered-Studio-Mobile/releases/tag/v0.4.0-dev.40
- ✅ **New exercise — `*` only dereferences a reference, E0614 (`textbook-integration`, 2026-07-08).** Audited two things from Paul's phone-test backlog first — starter comments (clean: the one "the fix is in X" scopes *where*, not *what*) and the exercise→lesson coupling (a perfect 67/67 bijection, zero dead links) — both solid, no defect. Then added a distinctive uncovered beginner wall: `*count` on a plain `i32` → E0614 "type `{integer}` cannot be dereferenced", teaching the mirror of `&` (`&` makes a reference, `*` follows one, so `*` is meaningless on a value you hold). Original-authored, guides without handing; new concept `dereference-non-reference` → 16-shared-references keeps the coupling a clean bijection (now 68/68). Verified: compile-checked wall + fix, `verify-exercises` 75/75, CONTENT_VERSION 40→41, live-drove Practice 390×844 (predict gate + concept chip + read-the-lesson → 16-shared-references), fmt + **e2e 83 passed**, seam-guard clean. Batched for v0.4, no APK.
- ✅ **Book → Patina lesson reverse switch — bidirectional now complete (`9a6c4ab`, 2026-07-08).** Completes the #41 lesson↔Book switching started the prior tick. A Rust Book chapter now offers the reciprocal of the lesson→chapter link: a prominent "📗 New to this topic? Learn it step-by-step in the Patina lesson →" at the top, for a beginner who lands on a dense canonical chapter. No new hand-maintained map — `BOOK_LESSON` is DERIVED from `LESSON_BOOK` (each chapter → its primary/lowest-numbered lesson: ch04-01→15-ownership, ch06-02→11-match-intro). 30 chapters mapped; the rest show none. Verified: build-time check (all 30 primaries real, sensible intros), live-drove 390×844 (ch04-01 link opens 15-ownership; ch13-04-performance shows none), added a lesson-nav reverse-switch test (9/9). e2e 82/83 — the miss is the pre-existing `tutor` compile flake (passes 1/1 isolated). seam-guard clean. GUI-only, no CV bump.
- ✅ **Lessons → topic-precise Rust Book chapter switching (`9372bd1`, 2026-07-08).** Continued the #41 textbooks headline (readability landed prior tick; this is the back-and-forth *switching* piece). Lessons could only reach the coarse per-phase PDF shelf — no path to the topic-precise in-app Rust Book chapters (just cleaned up last tick). Added a curated `LESSON_BOOK` map (Patina lesson → the bundled `book/` chapter on the same topic, reciprocal of the Book's book_refs) and made the lesson's book link smart: link to the exact in-app chapter when mapped (fast, offline), else fall back to the per-phase PDF. 55 mappings. Verified: build-time integrity check (all 55 keys real lessons, all values real chapters — no dead links); transform + JS-parse clean; live-drove 390×844 (15-ownership/18-defining-structs → precise chapter link lands on the right chapter; 17-slices-in-depth → PDF fallback); updated `lesson-nav.spec.js` to assert both paths; **e2e 82 passed**; seam-guard clean. GUI shell only — no CV bump. Works web + mobile.
- ✅ **Book — the offline Rust Book reads cleanly + honors offline-first (`76dd68c`, 2026-07-08).** Drove a headline item (#41 textbook readability) instead of micro-polish. Two defects: (1) since the code listings aren't bundled, `rpro-book` replaced each `{{#include}}` with "📖 Read this code listing in the Rust Book: <chapter-url>" — an ONLINE link repeated per listing, pointing an offline reader at the chapter they're already reading. Reworded the callout (in `rpro-book`, the crate shared by web + mobile) to lead with the OFFLINE action — write & run it in the Sandbox — and label the URL honestly as online. (2) mdBook `<Listing number caption>` tags passed through and the browser dropped them, losing the caption context; added `cleanBookMarkdown` in the GUI (runs web + mobile) that renders them as a bold caption + strips stray directives, and fixed the ToC blurb that admitted the leak. Verified: rpro-book 12/12 tests, cleaner clears all 33 chapters, transform + JS-parse clean, live-drove 390×844 (ch04-01 shows the "Listing 4-1" caption + 📝 Sandbox nudge, code fences keep Copy, zero raw directives), fmt + **e2e 82 passed**, seam-guard clean. No CV bump (book/*.md unchanged). Minor follow-up: one rare mixed-code-fence path still says "read it in the Rust Book."
- ✅ **IDE — editor edits now persist as an offline draft (`6ea9fe0`, 2026-07-07).** The fullscreen IDE loaded each file's disk content with NO local persistence, unlike Practice/Sandbox — so an unsaved edit was lost on any view-switch/reload/app-close, and on mobile it was ALWAYS lost (💾 Save is a PUT that Android's `shouldInterceptRequest` can't answer, so it silently failed with no fallback). Added the same draft safety net Practice uses, keyed per file (`ts-ide-buf:<id>`, separate from Practice's buffers): `ideRenderEditor` restores a saved draft over disk content and persists on every change (CM6 updateListener + textarea oninput); a buffer equal to disk clears its draft; a successful Save clears it too; a new ↺ Reset button discards the draft and reloads the file (escape hatch). 💾 Save still commits to disk on web; the draft is the always-available offline layer (and the only persistence on mobile). Verified: transform + JS-parse clean; live-drove 390×844 (typed a marker → reloaded → "draft restored ✏️" → Reset cleared it + restored disk); added an `ide.spec.js` round-trip regression test; **e2e 82 passed**; seam-guard clean. GUI shell only — no CV bump, no APK.
- ✅ **UI — the concept chip is now a clearly-tappable pill (`426f6ad`, 2026-07-07).** Follows last tick's glossary fix: every exercise concept resolves to a definition, but the chip was dotted-underline text lost among plain metadata, so on a phone (no hover) learners never discovered the definition it opens. Restyled `.gloss` as a small rounded pill (accent text, subtle themed fill + border) that reads as an interactive tag on touch, with an `.open` active state toggled while its definition card shows (cleared on ✕). Scoped safely (`.gloss` used once; `showGloss` called only from the chip). #40 beauty + discoverability, both themes, fully offline. Verified: gui-transform + JS-parse clean; live-drove Practice 390×844 (pill renders distinct, tap toggles active + opens the Mutability card); **e2e 81 passed** (incl. a11y contrast ×2 themes — one run showed a pre-existing `recall-chip`→`web.spec` store-pollution flake, green on re-run + web.spec passes in isolation); seam-guard clean. GUI shell only — no CV bump, no APK.
- ✅ **Glossary — every exercise concept chip now resolves (`599b940`, 2026-07-07).** Audited all 67 distinct exercise concepts against the offline glossary the Practice concept chip pulls from: 2 returned nothing — `tuple-field-access` and `array-length-type` (the two newest exercises' concepts) — so tapping their chip was a silent dead-end. Fixed with zero new content: aliased each orphan concept onto the general term that already teaches it (the "tuple" definition already covers `t.0`/`t.1` field access; the "array" definition already covers "length is part of the type"). Verified all 67 now resolve (0 missing); live-drove Practice at 390×844 — the tuple chip now opens the glossary card with the full definition + Book citation; CONTENT_VERSION 39→40; e2e **81 passed**; fmt + seam-guard clean. Batched for v0.4, no APK.
- ✅ **New exercise — struct literals can't add undeclared fields, E0560 (`textbook-integration`, 2026-07-07).** Rotated from two UI ticks to content. Added a beginner exercise for `struct-construction` (lesson 18): the mirror image of the existing missing-field (E0063) exercise — both real fields set correctly, but the literal invents an extra `admin` field, so `rustc` stops with E0560 "struct `User` has no field named `admin`". Teaches that a struct definition is a fixed contract the literal must match exactly. Original-authored, guides without handing the fix; reuses the already-mapped concept so the lesson-coupling bijection is unchanged. Verified: compile-checked wall + fix, `verify-exercises.sh` 74/74 (was 73), CONTENT_VERSION 38→39, live-drove Practice at 390×844 (predict gate + `struct-construction` chip + a "read the lesson" link resolving to `18-defining-structs`), full browser e2e **81 passed**, fmt + seam-guard clean. (Note: had to `npx playwright install chromium-headless-shell` — the e2e browser binary had gone missing.) Batched for v0.4, no APK.
- ✅ **IDE redesign — the file explorer is now PART of the IDE, fully offline (`883e762` TS + `1d13442` mobile, 2026-07-07).** Paul (emphatic): the fullscreen IDE was too toy-like — switching to the Files tab HID the editor, so it wasn't a real IDE, and everything must work offline. Reworked the 3-mutually-exclusive-tabs into a proper workbench: a file-explorer SIDEBAR beside an ALWAYS-VISIBLE editor with a foldable terminal below. On a phone the explorer is a slide-over drawer that steps aside the instant you open a file (editor never hidden); on desktop it docks left. New header (📂 Files / 🛠 IDE / open filename / ▤ Terminal / 🖥 Fullscreen), state persisted in `ts-ide-state`. Emoji hardened for the offline bundle: swapped 🗂→📂 and the tofu-prone ⛶→🖥 (fonttools cmap dump confirmed both are in the vendored Noto subset; ⛶/⤢ are genuinely absent), warmed on IDE-open to kill a first-paint flash. Rewrote `ide.spec.js` for the new DOM incl. the core regression (opening a file keeps the editor visible; folding the explorer never hides it) — **25 e2e pass** (IDE + a11y ×2 themes). **Mobile offline gap closed:** the phone's embedded server dropped `/api/workspace*` (dead file explorer offline) — added JNI `workspaceJson`/`workspaceFileJson` (mirrors rpro-serve, keyed by id → no `.toml` leak, no 423 gate on-device), routed the two GETs in MainActivity with a status-aware JSON helper; cargo-ndk built all 4 ABIs, symbols export, **APK assembles**. Held from release (batch to v0.4). Mobile push pending a GitHub 403 cooldown.
