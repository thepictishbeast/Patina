...
- [ ] 🔒 **P2 — Exercise click-to-select** (`POST /api/select`) — **deferred: needs a progress-model decision, not a quick endpoint.** `Progress` encodes "being worked on" as `ExerciseStatus::Current`, mutually exclusive with `Done`. A naive select that calls `set_current(id)` on a **Done** exercise (to revisit it) would silently strip its Done status and regress the done-count/gauge. Doing it right means decoupling a `current` *pointer* from per-exercise status across rpro-state + all three surfaces (set_current/resolve_current/record_run, web/TUI/CLI) — a larger, back-compat-sensitive refactor worth a deliberate pass or Paul's input, not a drive-by tick.
...
> **Audit status (#12): `docs/AUDIT.md`** — **103 tests / 0 fail** (refreshed t49), seam+wasm gates, **smoke 13/13** (incl. the Book reader's path-traversal guarantee), security review + release verify done. In-sandbox audit is GREEN + comprehensive; only the two 🔧 CI-gated browser audits above remain.
