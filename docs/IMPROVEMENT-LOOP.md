...
- ✅ **Educational-fidelity audit 2026-06-23** → `docs/audits/EDU-FIDELITY-2026-06-23.md`.
  ...
  **Verdict: the core pedagogy is faithfully implemented and works live** (predict-gate,
  ...
  (blocked, #14/#15); G6 editor pane clips framing comments (#25).- ✅ **dev.25 — Q pt.2 sweep, phase 10/concurrency (2026-07-07).** 4 thread/Arc/Mutex/channel exercises stripped of the fix-reveals: "reach for the thread-safe cousin" (Arc), "add the piece that allows safe mutation" (Mutex), the E0382 spoiler, and the CONCEPT + "print it before the send, or send a copy" walkthrough → tight task + constraint + read-the-error. Code byte-identical (diff comment-only), 71 verify (E0373/E0277/E0594/E0382 all fire). Web live (TS `b1df098`); mobile dev.25 (10339) prereleased. **10/11 done — only 09-advanced remains.** [[mobile-v04-testing-backlog]].
