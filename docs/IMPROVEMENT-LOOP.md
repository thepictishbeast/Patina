...
- ✅ **Educational-fidelity audit 2026-06-23** → `docs/audits/EDU-FIDELITY-2026-06-23.md`.
  ...
  **Verdict: the core pedagogy is faithfully implemented and works live** (predict-gate,
  ...
  (blocked, #14/#15); G6 editor pane clips framing comments (#25).- ✅ **Termux copy-command card + wrapped-bullet fix + layout paradigm chosen (2026-07-04).** Paul (phone) sent 3 things: (1) copy ONLY the Termux setup command → new #termuxSetup DOM card with a one-tap ⧉ Copy command button (idempotent one-liner, safe to re-run; persists — manifest RUN_COMMAND is a normal perm, Termux setting is a file) [`894cc79`]. (2) Learn bullets that wrap looked disconnected → mdToHtml now joins hard-wrapped bullet continuations into the <li> instead of a stray margin <p>; +2 tests; verified live [`d386aa8`]. (3) Big Practice-layout redesign directive → saved to memory, Paul CHOSE collapsible-panels; build order banked (console→problem→focus-code→ref drawer). OPEN: Paul's 'orange bar at the bottom' — couldn't repro in current build (gauge empty at 0%); need a screenshot. Both fixes flow to the phone app via build-apk.sh. NO APK.
